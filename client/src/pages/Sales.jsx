...
export default function Sales() {
  ...
  const fetchMasterData = async () => {
    const [b, g, c, t] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Sale recorded & challan created');
      setModalOpen(false);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 bg-white p-4 rounded-lg shadow">
        ...
        <input type="date" value={filters.endDate} onChange={(e) => setFilters({ ...filters, endDate: e.target.value })}
          className="px-3 py-2 border rounded-lg text-sm" />
        <button onClick={fetchSales} className="px-4 py-2 bg-gray-100 rounded-lg text-sm font-medium hover:bg-gray-200">Filter</button>
      </div>
      ...
      {/* New Sale Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="New Sale" size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Save Sale</button>
          </div>
        </form>
      </Modal>
    </div>
